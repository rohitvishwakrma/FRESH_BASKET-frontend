...
// import { AppContext } from "../context/appContext"; // your auth context
...
//   // Load initial state from user object
//   useEffect(() => {
//     if (user) {
...
//     setLoading(true);
//     try {
//       const { data } = await axiosInstance.put("/api/user/notifications", {
//         notificationsEnabled: !enabled,
//       });

//       if (data.success) {
//         setEnabled(data.notificationsEnabled);
//         setUser((prev) => ({ ...prev, notificationsEnabled: data.notificationsEnabled }));
//         toast.success(`Notifications turned ${data.notificationsEnabled ? "ON" : "OFF"}`);
//       } else {
...
//       <h1 className="text-3xl font-bold mb-6 text-center">Expiry Notification Settings</h1>

//       <p className="mb-6 text-center text-gray-600">
//         Receive email alerts when your grocery products are nearing their expiry date. 
//         Toggle the switch below to enable or disable notifications.
//       </p>
...
//           className={`relative inline-flex items-center h-12 w-24 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
...
//         Notifications are <span className="text-indigo-600">{enabled ? "Enabled" : "Disabled"}</span>
//       </p>

//       {loading && (
//         <p className="mt-4 text-center text-sm text-gray-500">Updating settings...</p>
//       )}
//     </div>
//   );
// };
...
const NotificationSettings = () => {
  ...
  const fetchExpiringProducts = async () => {
    ...
    try {
      const { data } = await axiosInstance.get("/api/products/expiring");
        ...
        {productsLoading ? (
          ...
          <div className="space-y-4">
            {expiringProducts.map(product => (
              <div key={product.id} className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm border border-gray-100">
                <div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500 mt-1">
              {enabled ? "You'll be notified when products are near expiry" : "Enable notifications to get alerts"}
            </p>
          </div>
        )}
      </div>
