    ...
    <div className="mt-16 w-full  bg-gray-50 py-10 px-2 md:px-8 rounded-xl shadow">
      ...
      <div className="my-6  grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 gap-6 items-center justify-center">
        {categories.map((category, index) => (
          <div
            key={index}
            className="group cursor-pointer py-6 px-4 rounded-xl gap-2 flex flex-col items-center justify-center bg-white shadow-md hover:shadow-lg transition-all border border-gray-100"
            onClick={() => {
          ...
          >
            <img
              src={category.image}
              alt=""
              className="max-w-24 transition group-hover:scale-110 drop-shadow object-contain  "
            />
